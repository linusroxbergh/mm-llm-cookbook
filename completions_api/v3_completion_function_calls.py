...
def call_edu_function(data: EducationalInput):
    filename = "educational_data.csv"
    if not os.path.exists(filename):
        with open(filename, "w") as f:
            writer = csv.writer(f)
            writer.writerow(["role", "education_level", "confidence"])
    
    with open(filename, "a") as f:
        writer = csv.writer(f)
        writer.writerow([data["role"], data["education_level"], data["confidence"]])
...
def llm_function_calls(user_prompt: str) -> str:
    ...
    if tool_calls:
        ...
        if tool_name == "handle_edu":
            ...
            call_edu_function(tool_arguments)
